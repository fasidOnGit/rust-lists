...
impl<T> List<T> {
    ...
    // Iter is around.
    pub fn iter(&self) -> Iter<T> { //Lifetime ellision...
...
impl<'a, T> Iterator for Iter<'a, T> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        self.next.map(|node| {
            // self.next = node.next.as_ref().map(|node| &**node); -> Can also be written as below with the turbofish! `::<&Node<T>, _>`
...
impl<T> List<T> {
    ...
    // creates the iter. Now &self needs to be valid as long as the
    // Iter is around.
    pub fn iter_mut(&mut self) -> IterMut<'_, T> { //Lifetime ellision...
